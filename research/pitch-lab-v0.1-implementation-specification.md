...
CI (canonical, §1.4/build spec) runs at freeze: T-INF1 suite only (infrastructure). At implementation milestones, the same workflow runs the growing suite — the workflow file itself does not change (CTest discovers tests). Perceptual listening tests never run in CI (human layer). Benchmark runs (RTF measurements) are CI-executed measurements recorded as artifacts, not pass/fail gates (except NaN/taint gates). Activation status (2026-09-26): the workflow is tracked in git, re-authored from the canonical spec and locally re-validated end-to-end on the pinned toolchain (3/3 green) after a workspace reset destroyed the original gitignored local copy; the owner has granted the PAT's Workflows permission, but the same reset lost the gitignored token value from `.env`, so the activation push is blocked locally at the credential check — build spec §12.1, OD-17. After activation it runs automatically on every push.
...
| **OD-17 (new)** | **CI activation on GitHub** — history: the fine-grained PAT initially lacked the `Workflows` permission (push rejected 2026-09-25, workflow kept locally and gitignored). 2026-09-26: the owner granted the permission, but a workspace reset destroyed the gitignored local `ci.yml` and the `.env` token values. The workflow was re-authored from the canonical build/CI spec (now **tracked in git**, with an empirical fix: `ctest --output-junit` paths resolve inside `--test-dir`) and locally re-validated (3/3 green on the pinned toolchain). Activation push blocked **locally**: `GITHUB_TOKEN`/`GITHUB_REPO` absent from `.env`. Exact chronology + one-time remediation: build spec §12.1 | owner restores the token values to `.env`, then one push activates CI automatically |
